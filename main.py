...
MENU_BG_TOP = (228,211,156)  # #E5E8F7 (light lavender)
MENU_BG_BOTTOM = (200, 209, 244)  # #C8D1F4 (pale periwinkle)
BUTTON_COLOR = (188,180,132)  # #8FA8F6
...
def draw_board(screen, state, font, back_button=None, logo_tagline=None, artifact_hint="", artifact_hint_timer=0, background_image=None):
    """
    Render the board with simplified display - only show AI thinking time.
    """
    # <<< CHANGED: Draw the background image first. If it doesn't exist, fill with white.
    if background_image:
...
def run_game_loop(screen, font, settings):
    ...
    # <<< NEW: Load and scale the background image for the game screen
    background_image = None
    scaled_background = None
    try:
        # Load the original image once
        background_image = pygame.image.load(os.path.join('assets', 'background.png')).convert()
        # Scale it to the current window size
        scaled_background = pygame.transform.scale(background_image, (CURRENT_WIDTH, CURRENT_HEIGHT))
    except pygame.error as e:
        print(f"Could not load background image from 'assets/background.png': {e}")

    try:
        score_bg = pygame.image.load(os.path.join('assets', 'score_bg.png')).convert_alpha()
        if score_bg:
            score_bg = pygame.transform.scale(score_bg, (int(200 * get_scale_factor()), int(60 * get_scale_factor())))

    except pygame.error as e:
        print(f"Could not load score background image: {e}")
        score_bg = None
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # <<< NEW: Rescale the background image if it exists
                if background_image:
                    scaled_background = pygame.transform.scale(background_image, (CURRENT_WIDTH, CURRENT_HEIGHT))
                            ...
                            for cell in new_state['edge_cells'][new_state['last_move']]:
                                if cell in new_state['claimed_items']:
                                    ...
                                    if item == "compass":
                                        ...
                                        artifact_hint_timer = pygame.time.get_ticks()
                        draw_board(screen, state, font, back_button, logo_tagline, artifact_hint, artifact_hint_timer, background_image=scaled_background)
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_g:
                ...
                if state['gauntlet_available'][0]:
                    ...
                    # <<< CHANGED: Pass the background image to draw_board
                    draw_board(screen, state, font, back_button, logo_tagline, artifact_hint, artifact_hint_timer, background_image=scaled_background)
                    continue
        ...
        if is_terminal(state):
            # <<< CHANGED: Pass the background image to draw_board for the final screen
            draw_board(screen, state, font, back_button, logo_tagline, artifact_hint, artifact_hint_timer, background_image=scaled_background)
